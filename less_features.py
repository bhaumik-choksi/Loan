...
# load data
loan = pd.read_excel('loan_1L.xlsx')
...
features = ['loan_amnt', 'term', 'int_rate',
            ...
            'annual_inc', 'verification_status', 'purpose', 'dti', 'open_acc', 'revol_bal',
            'total_acc', 'loan_status_coded']
...
# create model
model = Sequential()
model.add(Dense(35, input_dim=48, activation='relu'))
...
# model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
# model.load_weights('loan_weights.h5',by_name=True)
